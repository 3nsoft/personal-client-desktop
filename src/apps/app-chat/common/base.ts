...
export class JsonFromFile<T> extends StorageOnFS<client3N.WritableVersionedFS> {
  ...
  public async save(val: T, path?: string): Promise<void> {
    ...
    try {
      ...
      return await logError(`Error occured when saving json to ${this.mainFilePath}`, err);
    }
  }
  ...
  public async saveLink(file: web3n.files.ReadonlyFile, path?: string): Promise<void> {
    if (this.initializing) { await this.initializing; }
    const fullPath = !path
      ? this.mainFilePath
      : `${this.mainFilePath}/${path}`;
  ...
  public async readLink(fileName: string, path?: string): Promise<web3n.files.SymLink> {
    if (this.initializing) { await this.initializing; }
    const fullPath = !path
