...
export let ModuleName = "3nweb.components.notifications";
...
class Controller {
  ...
  delNotif(index: number): void {
    this.$timeout(() => {
      ...
      this.notifsSrv.writeNotifications(this.cacheSrv.notifications.list);
    })
  };
  ...
  runNotifAction(index: number): void { 
    ...
    if ((notif.actionData !== undefined) && (notif.type === "action")) {
      ...
      if (currentFolder === folderToTransfer) {
        ...
        this.$state.go("root.mail.folder", { folderId: folderToTransfer, msgMode: "show" });
      }
    }
  };
...
let componentConfig: angular.IComponentOptions = {
  bindings: {},
  templateUrl: "./templates/common/notifications/notifications.html",
  controller: Controller
}

export function addComponent(angular: angular.IAngularStatic): void {
  let mod = angular.module(ModuleName, [CacheSrvMod.ModulName, NotificationsSrvMod.ModulName]);
