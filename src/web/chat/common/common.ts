...
export class JsonFromFile<T> extends StorageOnFS<client3N.WritableVersionedFS> {
  ...
  save(val: T, path?: string): Promise<void> {
    if (!this.changeProc) {
    ...
    return this.changeProc.startOrChain(async () => {
      ...
      const fullPath = (!path) ? this.mainFilePath : `${this.mainFilePath}/${path}`
  ...
  async readLink(fileName: string, path?: string): Promise<web3n.files.SymLink> {
    if (this.initializing) { await this.initializing }
    const fullPath = (!path) ? `${this.mainFilePath}/${fileName}` : `${this.mainFilePath}/${path}/${fileName}`
